...
export class OpenAIProvider implements AIProvider {
  ...
  async extractResumeData(text: string): Promise<ExtractedResumeData> {
    ...
    return JSON.parse(completion.choices[0].message.content!) as ExtractedResumeData;
  }
  ...
  async streamExtractResumeData(
    ...
    return JSON.parse(fullText.replace(/```json\n?/g, "").replace(/```/g, "").trim()) as ExtractedResumeData;
  }
  ...
  async scoreResumeAgainstJob(
    ...
    const completion = await this.client.chat.completions.create({
      model: this.model,
      messages: [{ role: "user", content: this.buildScoringPrompt(resumeData, jobDescription, requiredSkills, bonusSkills) }],
      response_format: { type: "json_object" },
    });
...
${text}`;
  ...
  private buildScoringPrompt(resumeData: ExtractedResumeData, jd: string, required: string[], bonus: string[]): string {
