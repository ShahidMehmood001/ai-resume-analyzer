...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      ...
      "rounded-[var(--radius)] border border-[var(--color-input)]",
      "bg-[var(--color-surface)] px-3 py-2 text-sm text-[var(--color-foreground)]",
      "shadow-sm transition-colors",
      "focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)]",
      "disabled:cursor-not-allowed disabled:opacity-50",
      "[&[data-placeholder]>span]:text-[var(--color-muted-foreground)]",
      className,
    )}
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn(
      "py-1.5 pl-8 pr-2 text-xs font-semibold text-[var(--color-muted-foreground)]",
      className,
    )}
    {...props}
  />
